...
@cocoa(deps=[AppConfig, Database, ReaderRepository, LoanRepository])
class ReaderService:
    ...
    async def create_reader(self, request: CreateReaderRequest) -> ReaderResponse:
        ...
        async with self.database.begin() as session:
            ...
            if await self.reader_repository.get_by_card(session, card_no):
            ...
            await self.reader_repository.add(session, reader)
    ...
    async def search_readers(
        ...
        async with self.database.read() as session:
            readers, total = await self.reader_repository.search(
                session,
                keyword=keyword,
                status=status,
    ...
    async def delete_reader(self, reader_id: str) -> str:
        async with self.database.begin() as session:
            reader = await self.require_reader(session, reader_id)
            active = await self.loan_repository.count_active_for_reader(session, reader_id)
            if active:
                raise ConflictError(f"读者仍有 {active} 册未归还，无法注销")
            if reader.fine_balance_cents:
                raise ConflictError("读者仍有未缴罚金，无法注销")
            await self.reader_repository.delete(session, reader)
    ...
    def _to_response(self, reader: Reader, active: int, overdue: int) -> ReaderResponse:
        return ReaderResponse(
            ...
            max_loans=self.app_config.max_loans(reader.level),
            active_loans=active,
    ...
    async def _with_loan_counts(self, session, reader: Reader) -> ReaderResponse:
        ...
        overdue = await self.loan_repository.count_overdue_for_reader(
