...
class CirculationService(Canary):
    ...
    async def borrow(self, request: BorrowRequest) -> LoanResponse:
        ...
        async with self.database.begin() as session:
            ...
            book = await self.books.get(session, copy.book_id)
            if book is None:
            ...
            loan = Loan(
                ...
                reader_id=reader.id,
                borrowed_at=now,
                due_at=now + timedelta(days=self.config.loan_days(reader.level)),
                status="active",
            )
            await self.loans.add(session, loan)
            ...
            # 借走的正是为自己留存的那一册 → 预约兑现
            held = await self.reservations.find_open(session, book.id, reader.id)
    ...
    async def return_copy(self, request: ReturnRequest) -> ReturnReceipt:
        ...
        async with self.database.begin() as session:
            ...
            loan = await self.loans.active_for_copy(session, copy.id)
            if loan is None:
                raise ConflictError(f"副本 {copy.barcode} 当前没有在借记录")

            now = utcnow()
            overdue_days = _overdue_days(loan.due_at, now)
            fine = overdue_days * self.config.fine_cents_per_overdue_day
            ...
            reader = await self.readers.get(session, loan.reader_id)
            ...
            book = await self.books.get(session, loan.book_id)
    ...
    async def renew(self, loan_id: str) -> LoanResponse:
        async with self.database.begin() as session:
            loan = await self.loans.get(session, loan_id)
            if loan is None:
            ...
            if loan.renew_count >= self.config.max_renews:
                raise ConflictError(f"续借次数已达上限（{self.config.max_renews} 次）")
            if await self.reservations.count_open(session, loan.book_id):
                raise ConflictError("该书已有读者预约，无法续借")

            reader = await self.readers.get(session, loan.reader_id)
            level = reader.level if reader else "normal"
            loan.due_at = loan.due_at + timedelta(days=self.config.loan_days(level))
            loan.renew_count += 1
            session.add(loan)

            book = await self.books.get(session, loan.book_id)
            copy = await self.copies.get(session, loan.copy_id)
            return _to_loan(loan, book, copy, now)
    ...
    async def reader_loans(
        ...
        async with self.database.read() as session:
            ...
            loans, total = await self.loans.history_for_reader(
    ...
    async def overdue_loans(self, *, page: int, size: int) -> PageResult[LoanResponse]:
        async with self.database.read() as session:
            loans, total = await self.loans.list_overdue(
    ...
    async def reserve(self, request: ReserveRequest) -> ReservationResponse:
        async with self.database.begin() as session:
            ...
            book = await self.books.get(session, request.book_id)
            if book is None:
                raise NotFoundError(f"书目 {request.book_id} 不存在")

            if await self.reservations.find_open(session, book.id, reader.id):
            ...
            await self.reservations.add(session, reservation)
            position = await self.reservations.count_open(session, book.id)
            return _to_reservation(reservation, book, position)

    async def cancel_reservation(self, reservation_id: str) -> str:
        async with self.database.begin() as session:
            reservation = await self.reservations.get(session, reservation_id)
            ...
            if was_ready_copy:
                copy = await self.copies.get(session, was_ready_copy)
    ...
    async def reader_reservations(
        ...
        async with self.database.read() as session:
            reader = await self._require_reader(session, reader_id)
            rows, total = await self.reservations.list_for_reader(
                session, reader.id, offset=offset_of(page, size), limit=size
            )
            books = {
                b.id: b
                for b in await self.books.get_many(session, [r.book_id for r in rows])
            }
    ...
    async def _assert_borrowable(self, session, reader: Reader) -> None:
        ...
        overdue = await self.loans.count_overdue_for_reader(session, reader.id, utcnow())
        if overdue:
            raise ConflictError(f"有 {overdue} 册逾期未还，请先归还")
        if reader.fine_balance_cents >= self.config.max_fine_cents_before_block:
            raise ConflictError(
                f"欠款 {reader.fine_balance_cents} 分已达停借线，请先缴清罚金"
            )
        active = await self.loans.count_active_for_reader(session, reader.id)
        limit = self.config.max_loans(reader.level)
        if active >= limit:
    ...
    async def _resolve_copy(
        ...
        if copy_id:
            ...
            copy = await self.copies.get_by_barcode(session, barcode)
        if copy is None:
    ...
    async def _resolve_copy_for_borrow(
        ...
        copy = await self.copies.first_available(session, request.book_id)
        if copy is None:
            reserved = await self._reserved_copy_for(session, request.book_id, reader)
            if reserved is not None:
                return reserved
            counts = await self.copies.count_by_status(session, request.book_id)
            if not sum(counts.values()):
    ...
    async def _assert_copy_lendable(self, session, copy: BookCopy, reader: Reader) -> None:
        ...
        if copy.status == "reserved":
            held = await self.reservations.find_open(session, copy.book_id, reader.id)
            if held is not None and held.status == "ready" and held.copy_id == copy.id:
                return
    ...
    async def _reserved_copy_for(
        ...
        held = await self.reservations.find_open(session, book_id, reader.id)
        if held is None or held.status != "ready" or not held.copy_id:
            return None
        return await self.copies.get(session, held.copy_id)
    ...
    async def _release_or_hold(self, session, copy: BookCopy, now) -> bool:
        ...
        """
        waiting = await self.reservations.next_waiting(session, copy.book_id)
        if waiting is None:
        ...
        waiting.expires_at = now + timedelta(hours=self.config.reservation_hold_hours)
        session.add(waiting)
    ...
    async def _decorate(self, session, loans: list[Loan]) -> list[LoanResponse]:
        books = {
            b.id: b
            for b in await self.books.get_many(session, [ln.book_id for ln in loans])
        }
        now = utcnow()
        records = []
        for loan in loans:
